...
7. [Отмена платежа](#отмена-платежа)
8. [Проверка статуса](#проверка-статуса)
9. [Webhook уведомления](#webhook-уведомления)
...
**Пример запроса:**
```http
POST https://byfly-pay.com/api/payment/A3F5D9E2B1C8/cancel
X-API-Token: YOUR_64_CHAR_API_TOKEN
```
...
**Ограничения:**
- ❌ Нельзя отменить оплаченный платеж (`status = 'paid'`)
- ✅ Можно отменить: `pending`, `processing`, `partially_paid`, `failed`
- 🔄 Автоматическая отмена платежа на терминале Kaspi (если есть)
- 🔓 Автоматическое освобождение терминала

**Что происходит при отмене:**

1. Проверка что платеж не оплачен полностью
2. Отмена операции на терминале Kaspi (если активна)
3. Освобождение терминала
4. Обновление статуса на `cancelled`
5. Запись в лог транзакции
...
{
  "success": false,
  "message": "Нельзя отменить оплаченную транзакцию",
  "error_code": 400
}
```
...
- ⏰ Клиент передумал оплачивать
- 🔄 Нужно пересоздать платеж с другой суммой
- ❌ Платеж создан по ошибке
- 🚫 Истек срок действия счета
- 💸 Частичная оплата не подходит, нужно начать заново
...
const response = await fetch(
  'https://byfly-pay.com/api/payment/A3F5D9E2B1C8/cancel',
  {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
$result = $api->cancelPayment('A3F5D9E2B1C8');
...
// 2. Клиент передумал - отменяем
POST /api/payment/ABC123/cancel
...
// 4. Можно создать новый платеж с другой суммой
POST /api/payment/init
{
  "amount": 60000,
  "description": "Тур в Дубай (обновленная цена)"
}
```

### 6. Отмена при частичной оплате

```javascript
// 1. Платеж создан на 100000₸
POST /api/payment/init
{
  "amount": 100000
}

// 2. Клиент частично оплатил 50000₸
// Webhook
{
  "event": "partially_paid",
  "paid_amount": 50000,
...
// 3. Решили отменить и пересоздать
POST /api/payment/ABC123/cancel
...
// ВАЖНО! Уже оплаченные 50000₸ нужно вернуть клиенту
// Используйте функцию возврата средств
...
### 7. Автоматическая отмена истекших платежей

```javascript
// Получить список всех pending платежей старше 1 часа
async function cancelExpiredPayments() {
  const payments = await db.query(
    "SELECT transaction_id FROM payments WHERE status = 'pending' AND created_at < NOW() - INTERVAL 1 HOUR"
  );
  
  for (const payment of payments) {
    try {
      await api.cancelPayment(payment.transaction_id);
      console.log(`✅ Отменен истекший платеж: ${payment.transaction_id}`);
    } catch (error) {
      console.error(`❌ Ошибка отмены ${payment.transaction_id}:`, error.message);
    }
  }
}

// Запускать каждый час через cron
// 0 * * * * /path/to/cancel-expired.js
```

### 8. Проверка статуса в цикле
...
class ByFlyPaymentAPI {
    ...
    public function getPaymentMethods() {
        $response = $this->request('GET', '/payment-methods/active');
        return $response;
    }
...
class ByFlyPaymentAPI {
  ...
  async cancelPayment(transactionId) {
    return this.request('POST', `/payment/${transactionId}/cancel`);
  }
  ...
  async getPaymentMethods() {
    return this.request('GET', '/payment-methods/active');
  }
...
- [ ] Сохранен API токен в безопасном месте (переменные окружения)
...
### Отмена платежа
- [ ] Запрос на `/payment/{id}/cancel` с API токеном
- [ ] Обработка успешной отмены
- [ ] Обработка ошибки (платеж уже оплачен)
- [ ] Логирование отмененных платежей
...
- [ ] Тест отмены pending платежа
- [ ] Тест отмены processing платежа
- [ ] Попытка отменить paid платеж (должна вернуть ошибку)
